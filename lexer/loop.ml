...
let main() =
  ...
  while true do
    try
      ...
      let e = Parser.program Lex.lex lexbuf in
      let _ = Printf.printf "Recognized: " in
      let _ = List.iter (Ast.print_toplevel stdout )e  in
      (*let _ = Printf.fprintf stdout " =\n%!" in
      let _ = Pcfsem.printval (Pcfsem.eval e) in*) 
      Printf.printf "\n%!"
    with
      End_of_file -> Printf.printf "Reached end of input.\n%!"; exit 0
    |  Lex.Eoi -> Printf.printf  "Reached end of input.\n";
